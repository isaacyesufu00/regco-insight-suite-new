...
Deno.serve(async (req) => {
  ...
  if (error || !claims?.claims) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: corsHeaders });
  ...
  if (!body.name) return new Response(JSON.stringify({ error: "name required" }), { status: 400, headers: corsHeaders });
  ...
  if (cached && new Date((cached as any).expires_at) > new Date()) {
    return new Response(JSON.stringify({ ...((cached as any).result), cached: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
  ...
  if (!aiRes.ok) {
    ...
    return new Response(JSON.stringify({ error: `AI backend error ${aiRes.status}: ${t.slice(0, 200)}` }), { status: 500, headers: corsHeaders });
  }
